'use client';

import Image from 'next/image';
import { BackgroundSignup } from '@/public';

interface AuthBackgroundProps {
  className?: string; // Añadir className como propiedad opcional
}

const AuthBackground: React.FC<AuthBackgroundProps> = ({ className }) => {
  return (
    <div
      className={`relative hidden w-1/2 overflow-hidden lg:block lg:max-h-[1024px] lg:max-w-[720px] 2xl:max-h-[1080px] 2xl:max-w-[990px] ${className}`}
    >
      {/* Imagen de fondo (solo visible en lg+) */}
      <Image
        priority
        className="fixed h-full w-full object-fill lg:max-w-[720px] 2xl:max-w-[990px]"
        src={BackgroundSignup}
        alt="Auth page background"
      />
    </div>
  );
};

export default AuthBackground;
